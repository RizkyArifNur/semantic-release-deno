...
const getRefSha = require("./get-ref");
...
module.exports = async (pluginConfig, context) => {
  ...
  const reference = { owner, repo, ref: `refs/heads/v${gitTag}`, sha: await getRefSha() };
  ...
  if (!assets || assets.length === 0) {
    ...
    // Deno relase
    await github.git.createRef(reference);
    logger.log("Published Github's branch ", gitTag);
